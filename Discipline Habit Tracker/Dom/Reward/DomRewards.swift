...
//
//  Created by Wyatt Grant on 2023-10-24.
//

import SimpleToast
import SwiftUI

struct DomRewards: View {
  @Environment(\.scenePhase) private var scenePhase

  @State private var waiting = true
  @State private var showCreateRewardView = false
  ...
  @State private var localPoints = -999_999_999
        ...
        if waiting && rewards.count == 0 {
          ...
          ZStack {
            VStack(spacing: 0) {
              List {
                Stepper(
                  localPoints == -999_999_999 ? "Loading" : "\(localPoints) Points",
                  value: $localValue,
                  in: -999999...999999
                )
                .onChange(of: localValue) { oldValue, newValue in
                  Task {
                    await incrementPoints(add: newValue > oldValue)
                  }
                }
                ForEach(rewards) { reward in
                  RewardRow(reward: reward, showBank: true, showCost: true)
                ...
                .onDelete { indexSet in
                  ...
                  rewards.remove(atOffsets: indexSet)
              ...
              .navigationBarItems(
                trailing: Button(action: {
                  showCreateRewardView = true
                }) {
                  Image(systemName: "plus")
                  Text("Add")
                }
              )
              .navigationDestination(isPresented: $showCreateRewardView) {
                CreateEditReward(reward: $selectedReward, editMode: false)
              }
  ...
  func deleteReward() async {
    waiting = true
    var request = createAuthRequest(url: base_url + "/api/reward/" + String(selectedReward.id))
