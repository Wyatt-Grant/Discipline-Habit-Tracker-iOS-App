...
struct RewardDetails: View {
  ...
  @State private var showEditRewardView = false
  ...
  var body: some View {
        ...
        HStack {
          ...
          Button(action: {
            ...
            Text("Revoke")
              ...
              .background(waiting || reward.value <= 0 ? .gray : theme)
              .colorInvert()
              .cornerRadius(10)
              .contentShape(Rectangle())
          }
          .disabled(waiting || reward.value <= 0)
          .padding()
          Spacer()
          Button(action: {
            Task {
              ...
              HStack {
                Toggle(task.name, isOn: $switchStates[index])
                  .onChange(of: switchStates[index]) { oldValue, newValue in
                    Task {
                      await toggleRewardForTask(taskId: task.id, assign: switchStates[index])
                    }
                  }
              }
            }
    ...
    .navigationBarItems(
      trailing: Button(action: {
        showEditRewardView = true
      }) {
        Image(systemName: "pencil")
        Text("Edit")
      }
    )
    .navigationDestination(isPresented: $showEditRewardView) {
      CreateEditReward(reward: $reward, editMode: true)
    }
    .simpleToast(
      ...
      if let data = data {
        if let jsonData = try? JSONDecoder().decode(JsonTaskBase.self, from: data) {
          ...
          for task in jsonData.tasks ?? [JsonHabitTask]() {
            ...
            switchStates.append(reward.tasks.contains(task.id ?? 0))
          }
          waiting = false
        } else {
          showToast = true
  ...
  func toggleRewardForTask(taskId: Int, assign: Bool) async {
    waiting = true
    var request = createAuthRequest(
      url: base_url
        + "/api/\(assign ? "" : "un")assign-reward/\(String(reward.id))/\(String(taskId))")
