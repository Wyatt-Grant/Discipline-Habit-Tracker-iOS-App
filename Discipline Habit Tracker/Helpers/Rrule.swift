...
//
//  Created by Wyatt Grant on 2023-11-12.
//
...
struct RecurrenceParser {
  ...
  var days: [EKRecurrenceDayOfWeek]? {
    ...
    for byday in bydays {
      ...
      if byday.length > 2 {
        weekNumber =
          Int(byday.substring(with: NSRange(location: 0, length: byday.length - 2))) ?? -1
      ...
      if let _dofw = dofw {
        let dow =
          weekNumber > 0
          ? EKRecurrenceDayOfWeek(dayOfTheWeek: _dofw, weekNumber: weekNumber)
          : EKRecurrenceDayOfWeek(_dofw)
        days.append(dow)
      }
    }
  ...
  fileprivate func processRegexp(_ regexString: String) -> [NSString]? {
    ...
    if let match = reg?.firstMatch(
      in: String(self.ruleString),
      options: .reportProgress,
      range: NSRange(location: 0, length: self.ruleString.length))
    {
      return self.ruleString.substring(with: match.range(at: 1)).components(separatedBy: ",")
        as [NSString]?
    }
    return nil
  }
